...
class Healthguide extends CI_Controller {

    public function __construct() 
    {       
        parent:: __construct();
        if (!is_cache_valid(md5('healthguide'), 1800)){
            $this->db->cache_delete('healthguide');
            $this->db->cache_delete(md5('healthguide'));
        }
    }
    
    public function index() {
        $data["main_content"] = 'healthguide_view';
